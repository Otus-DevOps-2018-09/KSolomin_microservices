...
1. Когда мы создаем новую ветку в репозитории, у нас создается новый сервер. Сделано это с помощью добавления утилиты gcloud GCP в секцию новую секцию branch_start_vm .gitlab.yml файла. Для этого нам нужно в раннере привязывать Gitlab CI машину к нашему аккаунту GCP. Возможность удаления появляется благодаря.
